...
class _AddDeliveryAddressPageState extends State<AddDeliveryAddressPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
            title: Text("Crear dirección de envío", style: TextStyle(color: Colors.white, fontSize: 20.0, fontWeight: FontWeight.bold))
        ),
        body: Container(
  ...
  void saveAddress() async {
    ...
    if (checksFields()) {
      ...
      ResponseDto responseDto = await api.addDeliveryAddress(name.text, "1", address.text, city.text, cdgPostal.text, "1");

      if (NetworkUtils.isReqSuccess(responseDto.code)) {
        Utils.showToast("Dirección creada correctamente.", Colors.white, Colors.green);
        _emptyFields();
      } else {
        Utils.showToast("No se pudo crear la dirección.", Colors.white, Colors.red);
      }
    } else {
  ...
  bool checksFields() {
    return name.text != "" && address.text != "" && cdgPostal.text != "" && city.text != "";
  }
